...
engine = create_engine('postgresql://username:password@127.0.0.1:5432/radarr-main')
    ...
    for filename in os.listdir(directory):
        if filename.endswith('.csv'):
            with open(os.path.join(directory, filename), encoding="utf-8") as csv_file:
                ...
                print(table_name)
                if table_name == '"MovieMetadata"':
                    df["SecondaryYear"] = df["SecondaryYear"].astype(pandas.Int64Dtype())
                try:
